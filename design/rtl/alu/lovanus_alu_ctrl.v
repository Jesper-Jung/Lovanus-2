...
always @(*) begin
    ...
    if      ( ctrl_ALUOP_i == ALUOP_BRANCH  ) alu_ctrl = ALU_CTRL_SUB;
        ...
        case ({funct7_i, funct3_i})
            ...
            // I-Type
            {F7_ADDI,   F3_ADDI }  : alu_ctrl = ALU_CTRL_ADD;
            {F7_SLLI,   F3_SLLI }  : alu_ctrl = ALU_CTRL_SLL;
            {F7_SLTI,   F3_SLTI }  : alu_ctrl = ALU_CTRL_SLT;
            {F7_SLTUI,  F3_SLTUI}  : alu_ctrl = ALU_CTRL_SLTU;
            {F7_XORI,   F3_XORI }  : alu_ctrl = ALU_CTRL_XOR;
            {F7_SRLI,   F3_SRLI }  : alu_ctrl = ALU_CTRL_SRL;
            {F7_SRAI,   F3_SRAI }  : alu_ctrl = ALU_CTRL_SRA;
            {F7_ORI,    F3_ORI  }  : alu_ctrl = ALU_CTRL_OR;
            {F7_ANDI,   F3_ANDI }  : alu_ctrl = ALU_CTRL_AND;
        endcase
    end
end
